local path_sep = require("try").path_sep
...
function M.compile(flavour)
	local theme = require("try.lib.mapper").apply(flavour)
	local lines = {
		string.format(
			[[
...
vim.g.colors_name = "try-%s"
local h = vim.api.nvim_set_hl]],
			flavour == "beaver" and "light" or "dark",
			flavour
		),
	}
	...
	if not f then
		local err_path = (path_sep == "/" and "/tmp" or os.getenv "TMP") .. "/try_error.lua"
		print(string.format(
			[[Try (error): Most likely some mistake made in your try config
