...
if is_vim then require "try.lib.vim" end
...
local M = {
	...
	flavours = { beaver = 1, honeybee = 2, ant = 3, turtle = 4 },
...
function M.compile()
	...
	for flavour, _ in pairs(M.flavours) do
		M.flavour = flavour
		require("try.lib." .. (is_vim and "vim." or "") .. "compiler").compile(flavour)
	end
...
local function get_flavour(default)
	local flavour
	if default and default == M.flavour and vim.o.background ~= (M.flavour == "beaver" and "light" or "dark") then
	...
	if flavour and not M.flavours[flavour] then
		vim.notify(
			string.format(
				"Try (error): Invalid flavour '%s', flavour must be 'beaver', 'honeybee', 'ant', 'turtle' or 'auto'",
				flavour
			),
	...
	return flavour or M.options.flavour or vim.g.try_flavour or M.options.background[vim.o.background]
end
...
function M.load(flavour)
	...
	f(flavour or M.options.flavour or vim.g.try_flavour)
end

---@type fun(user_conf: TryOptions?)
function M.setup(user_conf)
	...
	local hash = require("try.lib.hashing").hash(user_conf)
...
vim.api.nvim_create_user_command(
	"Try",
	function(inp) vim.api.nvim_command("colorscheme try-" .. get_flavour(inp.args)) end,
	{
		nargs = 1,
...
vim.api.nvim_create_user_command("TryCompile", function()
	for name, _ in pairs(package.loaded) do
		if name:match "^Try." then package.loaded[name] = nil end
	end
	M.compile()
	vim.notify("Try (info): compiled cache!", vim.log.levels.INFO)
	vim.cmd.colorscheme "try"
end, {})

if vim.g.try_debug then
	vim.api.nvim_create_autocmd("BufWritePost", {
		pattern = "*/try/*",
		callback = function()
			vim.schedule(function() vim.cmd "TryCompile" end)
		end,
	})
end
