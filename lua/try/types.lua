---@class Try 
---@field options TryOptions
---@field setup fun(opts: TryOptions?)

---@alias CtpFlavor "turtle" | "ant" | "honeybee" | "beaver" | "auto"
...
---@class CtpFlavors<T>: {all: T, turtle: T, ant: T, honeybee: T, beaver: T }
...
-- By default try writes the compiled results into the system's cache directory.
...
-- Toggle integrations. Integrations allow Try to set the theme of various plugins.
---@field integrations CtpIntegrations?
-- Try colors can be overwritten here.
---@field color_overrides CtpColors | CtpFlavors<CtpColors<string>> | nil
-- Try highlights can be overwritten here.
...
---@class CtpBackground
-- Try flavor to use when `:set background=dark` is set.
---@field dark CtpFlavor?
-- Try flavor to use when `:set background=light` is set.
...
---@field enabled boolean
-- Set to a Try color name to use for the split separator.
